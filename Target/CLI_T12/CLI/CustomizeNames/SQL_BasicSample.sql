...
CREATE MATERIALIZED VIEW DataBaseName1.PUBLIC.JoinName AS SELECT
col1,
col2,
col3
FROM PUBLIC.atable;
...
$$
	...
	EXEC(`INSERT INTO PUBLIC.employee (empno,name,deptno,jobtitle,sex,dob,edlev) VALUES (:1, :2, :3, :4, :5, :6, :7)`,[NUMBER,NAME,DEPT,POSITION,SEX,DOB,EDLEV]);
	INSERT_TEMP(`SELECT
	*
	FROM PUBLIC.employee
	WHERE empno = :1`,[NUMBER]);
	return tablelist;
...
SELECT
MAX(CASE
WHEN (CNT_CF=1) THEN /*** MSC-WARNING - MSCEWI2001 - 'Object3' HAS RECURSIVE REFERENCES. FORWARD ALIAS CONVERSION COULD NOT BE COMPLETED ***/
 Object3
ELSE NULL END) OVER(PARTITION BY CIF_C
